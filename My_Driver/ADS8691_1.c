...
1011b = 1.25 × VREF
*/
#define COMPENSATE1 1

//#define VREF_0625          0x0004//输入的电压范围是+-0.625倍的参考电压2.53952
#define VREF_31         0x0000//输入的电压范围是正负3倍的参考电压12.2888
#define COM_WIT_RANGE_LSB1 0xD414//写命令的输入地址是14H的低16位，用来控制输入电压的范围
void ADS8691_Init1(void)//GPIO初始化，若cubemx配置过则不需
{
	/********************HAL库**************************/
...
//			SDI = 0;
		delay1();
		SDI1 = (COM_WIT_RANGE_LSB1 << i) >> 15;//写入控制命名表示要输入的是控制电压信号输入范围的寄存器
		CLK1 = 1; 
	}
...
//			SDI = 0;
	  ...
	  #ifdef VREF_31 
	    SDI1 = (VREF_31 << i) >> 15;//写入控制命名表示要输入的是控制电压信号输入范围的寄存器
	  #endif
		CLK1  = 1; 
	}
	 CS_AD86911=1;
